...
function mInitializeClassificationTab($ParentType, $file) {
	...
	if ($Global:mClsTabInitialized -ne $true) {
		...
		$Global:mClassCustentDef = $Global:mCustentDefs | Where-Object { $_.DispName -eq $UIString["Adsk.QS.ClassLevel_04"] } #"Class"
		...
		$global:mClsLevelNames = ($UIString["Adsk.QS.ClassLevel_00"], $UIString["Adsk.QS.ClassLevel_01"], $UIString["Adsk.QS.ClassLevel_02"], $UIString["Adsk.QS.ClassLevel_03"], $UIString["Adsk.QS.ClassLevel_04"] )
		$Global:mClassLevelCustentDefIds = ($Global:mCustentDefs | Where-Object { $_.DispName -in $mClsLevelNames }).Id

		#list all class level names for downstream reuse
		# add "Segment", "Main Group", "Group", "Sub Group", "Class", "Standard", "Term DE", "Code", "ID Number" "Comments", "Manual Input" "Comments DE"
		$global:mClsLevelProps = ($UIString["Adsk.QS.ClassLevel_00"], $UIString["Adsk.QS.ClassLevel_01"], $UIString["Adsk.QS.ClassLevel_02"], $UIString["Adsk.QS.ClassLevel_03"], $UIString["Adsk.QS.ClassLevel_04"], $UIString["Adsk.QS.ClassLevel_STD"], $UIString["ClassTerms_09"], $UIString["ClassTerms_10"], $UIString["ClassTerms_11"], $UIString["ClassTerms_12"], $UIString["Adsk.QS.ClassLevel_CID"], $UIString["LBL7"], $UIString["ClassTerms_17"], $UIString["ClassTerms_18"] )
	}
	...
	Switch ($ParentType) {
		"Dialog" {
			...
			if ($Prop["_XLTN_CLASS"].Value.Length -lt 1 -and $Prop["_ReadOnly"].Value -eq $false) { #alternate option: _XLTN_CLASSLINK 
				$dsWindow.FindName("btnRemoveClass").IsEnabled = $false
			...
			if ($Prop["_XLTN_CLASS"]) { #alternate option: _XLTN_CLASSLINK
				$dsWindow.FindName("txtActiveClass").Text = $Prop["_XLTN_CLASS"].Value #alternate option: _XLTN_CLASSLINK
			}
...
function mGetFileClsValues {
	...
	if ($AssignClsWindow) {
		...
		#Note - custom object names are not unique, only its Number, and we need to handle returning more than one.
		$mActiveClass += mGetCustentiesByName -Name $Prop["_XLTN_CLASS"].Value #alternate option: _XLTN_CLASSLINK
	}

	if ($mActiveClass.Count -eq 1 -or $mActiveClass.Count -eq 2) #Uniclass system may return two objects as it uses terms and class objects in parallel
	{
		...
		Foreach ($mClsProp in $mClsPrpNames.GetEnumerator()) {
			...
			if ($mClsPrpNames[$mClsProp.Key] -notin $global:mClsLevelProps) {
				$mClsPropTable.Add($mClsPrpNames[$mClsProp.Key], (($mFileClassProps | Where-Object { $_.PropDefId -eq ($mClsProp.Key) }).Val))
			}
			if ($dsWindow.Name -eq "FileWindow") {
				if ($mClsPrpNames[$mClsProp.Key] -eq $UIString["Adsk.QS.ClassLevel_00"]) { # "Segment"  
				...
				if ($mClsPrpNames[$mClsProp.Key] -eq $UIString["Adsk.QS.ClassLevel_01"] ) { # "Main Group" 
				...
				if ($mClsPrpNames[$mClsProp.Key] -eq $UIString["Adsk.QS.ClassLevel_02"]) { # "Group" 
				...
				if ($mClsPrpNames[$mClsProp.Key] -eq $UIString["Adsk.QS.ClassLevel_03"]) { # "Sub Group" 
				...
				if ($mClsPrpNames[$mClsProp.Key] -eq $UIString["Adsk.QS.ClassLevel_STD"]) { # "Standard" 
...
function mGetClsDfltValues {
	...
	if ($mActiveClass.Count -eq 1) {
		Foreach ($mClsProp in $mClsPrpNames.GetEnumerator()) {
			...
			if ($mClsPrpNames[$mClsProp.Key] -notin $global:mClsLevelProps) { $mClsPropTable.Add($mClsPrpNames[$mClsProp.Key], $mClsPrpValues[$mClsProp.Key]) }
		}
	}
...
function mGetCustentiesByName([String]$Name) {
	...
	$srchCond2.PropDefId = ($Global:mAllCustentPropDefs | Where-Object { $_.DispName -eq $UIString["Adsk.QS.ClassLevel_STD"] }).Id
...
function mApplyClassification() {
	if ($Global:mFile) {
		...
		# allow class assignment to all levels, except the root (required for Uniclass and custom standars)		
		$mActiveClass += mFindCustent -CustentName $dsWindow.FindName("txtActiveClass").Text -Category $UIString["Adsk.QS.ClassLevel_04"]
		...
		If ($mActiveClass.Count -eq 1) {			
			...
			Foreach ($mClsProp in $mClsPrpNames.GetEnumerator()) {
				#filter the all classification level properties but add all class' properties
				if ($mClsProp.Value -notin $global:mClsLevelProps) { $mPropsAdd += $mClsProp.Key }
			}
		}
		elseif ($mActiveClass.Count -gt 1) {
			...
			[Autodesk.DataManagement.Client.Framework.Forms.Library]::ShowError($UIString["Adsk.QS.Classification_16"], "VDS Sample Configuration")
			return
		}

		# save the current class id, if the ADSK.QS.EditFile.ps1 should create a link
		#$mActiveClass.Id | Out-File "$($env:appdata)\Autodesk\DataStandard 2025\mFileClassId.txt"
...
function mRemoveClassification() { #applies to $dsWindow
	...
	if ($Prop["_EditMode"]) {
		if ($Global:mFile) {
			...
			$mActiveClass = @()
			$mActiveClass += mFindCustent -CustentName $Prop["_XLTN_CLASS"].Value -Category $UIString["Adsk.QS.ClassLevel_04"] #"Class" #custom object names should be unique within a category, only its Number
			If ($mActiveClass.Count -eq 1) {
				$mClsPrpNames = mGetClsPrpNames -ClassId $mActiveClass.Id
				$mPropsRemove = @()
...
function mResetClassSelection {
	$mBreadCrumb = $AssignClsWindow.FindName("wrpClassification2")
	$children = @()
	$children += mGetCustentClsLevelList -ClassLevelName $UIString["Adsk.QS.ClassLevel_00"] # "Segment"
	...
	if ($mCompClsInitialized -ne $true) {
		...
		if ($AssignClsWindow.FindName("wrpClassification2")) {
			...
			if ($AssignClsWindow.FindName("wrpClassification2").Children.Count -lt 1) {
				...
				mAddClsLevelCombo -ClassLevelName $UIString["Adsk.QS.ClassLevel_00"] #"Segment"
			}
		}
	}
