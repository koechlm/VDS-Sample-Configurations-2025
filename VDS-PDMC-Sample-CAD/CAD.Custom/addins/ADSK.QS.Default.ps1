...
function InitializeWindow {             
	...
	#Copy Parent Project Number to file property "Project Number" if exists; be careful, not all dialogs might have the textbox control 'txtVaultLocation', e.g. DA, FG,...
	If ($Prop["Project"]) {
		$Global:mPropTrans = mGetPropTranslations
		$Global:mUIStrings = mGetUIStrings
		if ($dsWindow.FindName("VaultLocationPath")) {
			
			#build a name/value map assigning each target property name the source's property name
			$Global:mFld2FileMap = @{ $mPropTrans["PROJECT"] = "Name"; $mPropTrans["PROJECTNUMBER"] = "Project Number"}

			#the control VaultLocationPath is empty during initialization and must dynamically change if the user selects another folder
			$dsWindow.FindName("VaultLocationPath").add_TextChanged({		
					$mSrc = mGetNewFileParentFldrByCat($mUIStrings["CAT6"]) #get the next folder of category "Project" iterating hierarchy inversely
	...
	if ($Prop["_CopyMode"].Value -eq $true) {
		#disabled as it causes another read-only edit dialog on drawing copies		
		$Prop["Source File"].Value = $DC.PathAndFileNameHandler.OriginalFilename
	}
	...
	If ($dsWindow.FindName("expTermSearch")) {			
		Try {
			Import-Module -FullyQualifiedName "C:\ProgramData\Autodesk\Vault 2025\Extensions\DataStandard\Vault.Custom\addinVault\ADSK.QS.CustomObjectsClassified.psm1"
		}
		catch {}
	}
	...
	If ($dsWindow.FindName("expIEC61355")) {			
		Try {
			Import-Module -FullyQualifiedName "C:\ProgramData\Autodesk\Vault 2025\Extensions\DataStandard\Vault.Custom\addinVault\ADSK.QS.IEC61355.psm1"
		}
		catch {}
	}
	#endregion IEC61355
...
function InitializeInventorCategory {
	...
	switch ($mDocType) {
		'12291' {
			...
			If ($mDocSubType -eq "{28EC8354-9024-440F-A8A2-0E0E55D635B0}") {
				#weldment assembly
...
function mAddShortCutByName([STRING] $mScName) {
	...
	try {
		...
		$mImageNode = $mImageNode -replace $regex, "[A=$($mFldrColor.A), R=$($mFldrColor.R), G=$($mFldrColor.G), B=$($mFldrColor.B)]$($currentTheme)"
